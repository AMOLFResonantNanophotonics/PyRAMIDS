...
def plot_polar(P, thelist, folder, title, Pref= None):
    
    f,ax = plt.subplots(1,1,figsize=(5,10),subplot_kw=dict(projection="polar"), dpi=300)
    
    f.subplots_adjust(hspace=0.15)        
    ax.plot(thelist,P/np.max(P),'r', label = r'arbitrary dipole')
    
    if Pref is not None:
        ax.plot(thelist, Pref / np.max(Pref), ls='--', color='k', lw=2, label='reference')
    ...
    # ax.legend(loc='center left', bbox_to_anchor=(1.15, 0.5), frameon=False, fontsize=12)   
...
plt.xlim([0.075,0.6])
file = [folder,' Fig_5_LDOS'+'.pdf']
savefig(file[0], file[1])
plt.show()
plt.show()

#%% Position of dipole.. infront of the silicon layer
z=+dstack[0] + dstack[0]/2
...
kx, ky, S0_x, S1_x, S2_x, S3_x = vector.BFPplotpassport(theta_d,-phi_d,Es_down_px,Ep_down_px,nstack[0],title=' ',basis='cartesian', plot=False) 

plot(S0_x, S1_x, S2_x, S3_x, folder, 'Fig_5_spinning_electricdipole')
print(np.max(S0_x))
...
plot(S0_x, S1_x, S2_x, S3_x, folder, 'Fig_5_spinning_magneticdipole')
print(np.max(S0_x))
...
plot(S0_x, S1_x, S2_x, S3_x, folder, 'Fig_5_kerkerdipole')
print(np.max(S0_x))
...
plot(S0_x, S1_x, S2_x, S3_x, folder, 'Fig_5_magnetoelectric_pos_m1j_dipole')
print(np.max(S0_x))
...
kx, ky, S0_x, S1_x, S2_x, S3_x = vector.BFPplotpassport(theta_d,-phi_d,Es_down_px,Ep_down_px,nstack[0],title=' ',basis='cartesian', plot=False) 
plot(S0_x, S1_x, S2_x, S3_x, folder, 'Fig_5_magnetoelectric_neg_m1j_dipole')
print(np.max(S0_x))

#%%
'''POLAR PLOTS'''
print('doing POLAR PLOTS')
#%%
Nthe=501
...
#%%px 
'''spining electric dipole dipole'''
pu=[1.0,1j,0.0]  # electric 
mu=[0.0,0.0,0.0]  # magnetic 
 
P_d,outE,[theta_d,phi_d]=Farfield.RadiationpatternPandField(k0,z,pu,mu,thelist,0*thelist,nstack,dstack)

nstack0=[1., 1., 1.]
P_d0,outE,[theta_d,phi_d]=Farfield.RadiationpatternPandField(k0,z,pu,mu,thelist,0*thelist,nstack0,dstack)

plot_polar(P_d, thelist, folder, 'Fig_5_spinning_electricdipole', P_d0)


#%%px 
'''spining magnetic dipole dipole'''
mu=[1.0,1j,0.0]  # electric 
pu=[0.0,0.0,0.0]  # magnetic 
...
plot_polar(P_d, thelist, folder, 'Fig_5_spinning_magneticdipole', P_d0)

#%% 
'''Kerker dipole'''
pu=[1.0, 0.0, 0.0]  # electric 
mu=[0.0, 1.0, 0.0]  # magnetic 
...
plot_polar(P_d, thelist, folder, 'Fig_5_kerkerdipole', P_d0)

#%%
'''Magneto-electric Dipole'''
pu=[1.0,0.0,0.0]  # electric 
mu=[0.0,1j,0.0]  # magnetic 
...
plot_polar(P_d, thelist, folder, 'Fig_5_magnetoelectric_pos_m1j_dipole', P_d0)


#%%
pu=[1.0,0.0,0.0]  # electric 
mu=[0, -1j,0.0]  # magnetic 
...
plot_polar(P_d, thelist, folder, 'Fig_5_magnetoelectric_neg_m1j_dipole', P_d0)
