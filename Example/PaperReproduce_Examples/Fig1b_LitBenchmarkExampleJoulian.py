...
plt.ylabel(r"Density of states $\rho$")
plt.legend()
...
plt.loglog(omega_values, rho[:, 2], 'k-', label=r"$\rho_E(\omega) + \rho_M(\omega)$")  # Total LDOS


plt.xlabel(r"$\omega$ (rad s$^{-1}$)")
plt.ylabel(r"Density of states $\rho$")
plt.legend()
plt.tick_params(axis='both', direction='in', which='major', length=4)  # Major & minor ticks inside
plt.tick_params(axis='both', direction='in', which='minor', length=3)  # Major & minor ticks inside

# plt.grid(True, which="both", linestyle="--", linewidth=0.5)
file = [folder,'Fig_1b_Joulian_rhoEandM'+' .pdf']
