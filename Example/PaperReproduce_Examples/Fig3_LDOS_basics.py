...
lam = 1
k0= 2*np.pi/ lam
nstack = [1.5, 3.5, 1] # Si (n=3.5) layer sandwiched between glass (n=1.5) and air (n=1.)
dstack = [0.1*lam]

#%%
k0list=np.linspace(4,16,1000)
...
cbar.set_label(r'$\log_{10}$ LDOS($E_\parallel$) integrand', fontsize=12)
...
ax.set_title(r'log10 LDOS($E_\parallel$) integrand')

ax.set_ylim([-0.5, 0.6])
yticks = [-0.6, -0.3, 0, 0.1, 0.4, 0.7]
ax.set_yticks(yticks)
...
cbar = fig.colorbar(pcm, ax=ax)
cbar.set_label(r'$\log_{10}$ LDOS($E_\perp$) integrand', fontsize=12)
ax.plot(kparlist, 0 * kparlist, 'w')  
ax.plot(kparlist, 0 * kparlist + dstack[0], 'w') 
ax.plot([nstack[0], nstack[0]], [-1.0, 0.0], 'w--')
ax.plot([nstack[1], nstack[1]], [0, dstack[0]], 'w--')
...
out=ImGLDOS.LDOSintegrandplotdispersion(k0list,kparlist,dstack[0]/3,nstack,dstack)

#%%
fig, ax = plt.subplots(figsize=(6, 5), dpi=350)

## make plot coordinates
kparonk0,k0=np.meshgrid(kparlist,k0list)
k0=k0*dstack[0]/(2.0*np.pi)
kpar=kparonk0*k0
kparline=k0list*dstack[0]/(2.0*np.pi)

pcm = ax.pcolor(kpar, k0, np.log10(np.abs(out[0,:,:])), cmap = 'inferno', shading='auto', rasterized= True)
ax.patch.set_facecolor('black')
cbar = fig.colorbar(pcm, ax=ax)
cbar.set_label(r'$\log_{10}$ LDOS($E_\parallel$) integrand', fontsize=12)

ax.plot(kparline*nstack[0], kparline, 'w')  
ax.plot(kparline*nstack[1], kparline, 'w') 
ax.plot(kparline*nstack[2], kparline, 'w') 

pcm.set_clim([-2, 2])
ax.set_xlabel(r'$k_{\parallel} d /2\pi$')
ax.set_ylabel(r'Frequency (d/$\lambda$)')
ax.set_title(r'log10 LDOS($E_\parallel$) integrand')
 


yticks = [0.08,0.16,0.24]
ax.set_yticks(yticks)
...
ax.tick_params(axis='both', direction='in', length=4, width=1)
file = [folder,'Fig_3d_LDOSdispersion_parallel'+'.pdf']
savefig(file[0], file[1])
plt.show()
...
pcm = ax.pcolor(kpar, k0, np.log10(np.abs(out[1,:,:])), cmap = 'inferno', shading='auto',rasterized= True)
ax.patch.set_facecolor('black')
cbar = fig.colorbar(pcm, ax=ax)
cbar.set_label(r'$\log_{10}$ LDOS($E_\perp$) integrand', fontsize=12)
...
ax.set_xlabel(r'$k_{\parallel} d /2\pi$')
ax.set_ylabel(r'Frequency (d/$\lambda$)')
ax.set_title(r'log10 LDOS($E_\perp$) integrand')
...
ax.tick_params(axis='both', direction='in', length=4, width=1)
file = [folder,'Fig_3e_LDOSdispersion_perpendicular'+'.pdf']
savefig(file[0], file[1])
plt.show()
