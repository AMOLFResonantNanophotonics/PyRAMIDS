...
freqlist = np.linspace(0.001, 0.25, 251) *(2*np.pi/a)
kparlist = np.linspace(0, 0.6, 401)  *(2*np.pi/a)
...
reflectance_full = np.vstack((np.flip(rp, axis=0), rs))
...
pcm = ax.pcolormesh(KX_FULL, FREQ_FULL, reflectance_full, cmap="gray_r", shading='auto', vmin=0, vmax=1)

ax.plot(light_line, freqlist, 'k-', label="Light Line")
ax.plot(-light_line, freqlist, 'k-', label="Light Line")

cbar = fig.colorbar(pcm, label="R")
