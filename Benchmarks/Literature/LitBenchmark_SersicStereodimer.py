...
def magnetoelectric_polarizability(omega, omega0, gamma, V, etaE, etaH, etaC, nsur):
    ...
    # Fudging to ensure invertibility
    alpha0 += 1e-14 * Lorentz * np.eye(6)
    return alpha0
...
twist_angles = np.linspace(0, 180, 51)

lamlist = np.linspace(1000, 3000, 61)
...
pcm = ax.pcolormesh(twist_angles, frequencies_THz, ext_vs_twist.T*1e-6, vmin = 0., vmax = cmax, shading='gouraud', cmap='inferno', rasterized = True)
